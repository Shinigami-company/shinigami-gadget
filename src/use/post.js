...
class Report {
  ...
  async post(
    ...
    const do_fill_field = (filed_key) =>
      if_fill_field(filed_key)
        ? translate(
            lang,
            `post.${this.translate_key}.${filed_key}`,
            translate_replace
          )
        : null;
    ...
    await DiscordRequest(this.webhook_url, msg);
  }
}
...
export const webhook_reporter = (() => {
  ...
  for (const v of reporter_list) {
    dic[v] = new Report(v, process.env[`webhook_${v}`], {
      content: false,
      embed: true,
      title: true,
      description: true,
