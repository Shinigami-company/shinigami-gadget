...
export class Achievement {
	...
	graduations;
	rewards;
	constructor(name, modelKey, maxLevel=1, hidden=false, graduations=undefined, rewards=undefined) {
	...
	{
		const received_level=await api.KiraUserAchiv.findOne(f_achivModelId, {select: {[this.modelKey]: true}}).then(obj => obj[this.modelKey]);
	...
	{
		await api.KiraUserAchiv.update(f_achivModelId, {[this.modelKey]: f_setLevel});
	}
	...
	async do_check(f_userModel, f_value, f_lang, f_doneDolarValues={})
	{
		...
		return await this.do_grant(f_userModel, f_lang, this.#level_graduate(f_value), f_doneDolarValues);
	}
	async do_grant(f_userModel, f_lang, f_newLevel=1, f_doneDolarValues={})
	{
		...
		{//if has a greater level
			console.log(`LOG : achiv.js : model id [${f_userModel.statPtr.id}]  pass [${this.name}] from ${f_newLevel} to ${h_registerLevel}`);
			//set the level
			...
			//message
			{
				...
				let sending_content=translate(f_lang, `achievement.done.yay.${yayTypeStr}`, {name: translate(f_lang, `achievements.${this.name}.title`), level: roman_from_int(f_newLevel)});;
				const doneMessage=translate(f_lang, `achievements.${this.name}.done`, f_doneDolarValues);
				sending_content+="\n"+doneMessage;
...
new Achievement("test2", "level_test2", 8, false, [5, 10, 20, 50, 100, 200, 500, 1000], [1, 2, 3, 4, 5, 6, 7, 8]);

new Achievement("kill", "level_kill", 5, false [5, 10, 20, 50, 100]);
new Achievement("counter", "level_counter", 5, false, [5, 10, 20, 50, 100]);
new Achievement("outerTime", "level_outerTime", 5, false, [1*3600, 5*3600, 10*3600, 50*3600, 100*3600]);
...
new Achievement("killDailyStreak", "level_killDailyStreak", 3, false, [3, 7, 30]);
